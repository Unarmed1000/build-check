...
def analyze_cross_library_dependencies(header_to_headers: Dict[str, Set[str]],
                                       header_to_lib: Dict[str, str]) -> CrossLibraryAnalysis:
...
def parse_ninja_libraries(build_ninja_path: str) -> Tuple[Dict[str, Set[str]], Dict[str, Set[str]], Set[str], Set[str]]:
    ...
    logger.info(f"Parsing {build_ninja_path}...")
    ...
    try:
        ...
        logger.error(f"Failed to read {build_ninja_path}: {e}")
        raise
    
    logger.info(f"Found {len(all_libs)} libraries and {len(all_exes)} executables")
...
def compute_library_metrics(lib_to_libs: Dict[str, Set[str]], 
    ...
    try:
        ...
        logger.debug(f"Could not compute transitive dependents: {e}")
...
def find_unused_libraries(lib_to_libs: Dict[str, Set[str]], 
                          exe_to_libs: Dict[str, Set[str]],
                          all_libs: Set[str]) -> Set[str]:
    ...
    logger.info(f"Found {len(unused)} unused libraries")
    return unused
...
def find_library_cycles(lib_to_libs: Dict[str, Set[str]]) -> List[Set[str]]:
    ...
    # Convert to include graph format
    include_graph = {lib: deps for lib, deps in lib_to_libs.items()}
    ...
    cycles = find_strongly_connected_components(G)
    ...
    for i, part in enumerate(parts):
        if part in ('DemoFramework', 'DemoApps'):
            if i + 1 < len(parts):
                ...
                elif part == 'DemoApps' and i + 2 < len(parts):
                    category = parts[i + 1]
                    app = parts[i + 2]
