...
if sys.version_info < (3, 8):
    print(f"Error: Python 3.8 or higher is required. You have Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}", file=sys.stderr)
...
def check_all_packages() -> bool:
    ...
    try:
        is_installed, meets_version, installed_ver = check_package_version(
            'packaging', 
            PACKAGE_REQUIREMENTS['packaging'],
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description='Verify build-check package dependencies',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument(
        '--check-all',
        action='store_true',
        help='Check all known runtime packages'
    )
