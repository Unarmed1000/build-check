...
require_package('networkx', 'library graph analysis')
...
def compute_library_metrics_internal(lib_to_libs: Dict[str, Set[str]], 
...
Examples:
  ...
  %(prog)s ./build --exclude "*/ThirdParty/*" --exclude "*Test*"
    ...
    if not os.path.exists(build_ninja_path):
        print(f"{Colors.RED}Error: build.ninja not found in '{args.build_dir}'{Colors.RESET}", 
              file=sys.stderr)
