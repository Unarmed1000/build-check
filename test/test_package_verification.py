...
def test_check_package_version_installed_and_current() -> None:
    ...
    is_installed, meets_version, installed_ver = check_package_version("networkx", "2.8.8", raise_on_error=False)
...
def test_check_package_version_missing_package() -> None:
    ...
    with patch("lib.package_verification.version") as mock_version:
        ...
        is_installed, meets_version, installed_ver = check_package_version("nonexistent-package", "1.0.0", raise_on_error=False)
...
def test_check_package_version_old_version() -> None:
    ...
    with patch("lib.package_verification.version") as mock_version:
        ...
        # Should not raise with raise_on_error=False
        is_installed, meets_version, installed_ver = check_package_version("some-package", "3.0.0", raise_on_error=False)
...
def test_check_package_version_uses_registry() -> None:
    ...
    with patch("lib.package_verification.version") as mock_version:
        ...
        is_installed, meets_version, installed_ver = check_package_version("networkx", min_version=None, raise_on_error=False)  # Use registry
...
def test_version_comparison_semantic() -> None:
    ...
    with patch("lib.package_verification.version") as mock_version:
        ...
        is_installed, meets_version, _ = check_package_version("test-pkg", "3.9.0", raise_on_error=False)

        assert meets_version is True  # 3.10.0 >= 3.9.0

        # Test that 3.10.0 < 3.11.0
        is_installed, meets_version, _ = check_package_version("test-pkg", "3.11.0", raise_on_error=False)
