...
class TestPrintSummaryStatistics:
    ...
    def test_print_summary_with_cycles(self, capsys: Any) -> None:
        ...
        stats = MatrixStatistics(
            total_headers=10,
            total_actual_deps=25,
            total_possible_deps=90,
            sparsity=72.2,
            avg_deps=2.5,
            health="Moderately coupled",
            health_color="\033[93m"
        )
        
        print_summary_statistics(
            stats=stats,
            cycles_count=3,
            headers_in_cycles_count=7,
            layers=[],
            has_cycles=True
        )
...
class TestPrintCircularDependencies:
    ...
    def test_print_cycles_found(self, capsys: Any, tmp_path: Path) -> None:
        ...
        cycles = [
            {str(tmp_path / 'A.hpp'), str(tmp_path / 'B.hpp')},
            {str(tmp_path / 'C.hpp'), str(tmp_path / 'D.hpp'), str(tmp_path / 'E.hpp')},
        ]
        
        feedback_edges = [
            (str(tmp_path / 'A.hpp'), str(tmp_path / 'B.hpp')),
            (str(tmp_path / 'C.hpp'), str(tmp_path / 'D.hpp')),
        ]
    ...
    def test_print_many_cycles(self, capsys: Any, tmp_path: Path) -> None:
        ...
        # Create 15 cycles  
        cycles = [
            {str(tmp_path / f'H{i}.hpp'), str(tmp_path / f'H{i+1}.hpp')}
            for i in range(15)
        ]
...
class TestPrintLayeredArchitecture:
    ...
    def test_print_layers_with_auto_display(self, capsys: Any, tmp_path: Path) -> None:
        ...
        layers = [
            [str(tmp_path / 'Top.hpp')],
            [str(tmp_path / 'Bottom.hpp')],
        ]
...
class TestPrintHighCouplingHeaders:
    ...
    def test_print_high_coupling(self, capsys: Any, tmp_path: Path) -> None:
        ...
        headers = [
            str(tmp_path / 'HighCoupling.hpp'),
            str(tmp_path / 'MediumCoupling.hpp'),
            str(tmp_path / 'LowCoupling.hpp'),
        ]
...
class TestPrintRecommendations:
    ...
    def test_recommendations_with_cycles(self, capsys: Any) -> None:
        ...
        cycles = [{'A.hpp', 'B.hpp'}]
        metrics = {
            'A.hpp': DSMMetrics(fan_in=1, fan_out=1, coupling=2, stability=0.5),
            'B.hpp': DSMMetrics(fan_in=1, fan_out=1, coupling=2, stability=0.5),
        }
        all_headers = {'A.hpp', 'B.hpp'}
        stats = MatrixStatistics(
            total_headers=2,
            total_actual_deps=2,
            total_possible_deps=2,
            sparsity=50.0,
            avg_deps=1.0,
            health="Moderate",
            health_color=""
    ...
    def test_recommendations_high_coupling(self, capsys: Any) -> None:
        ...
        metrics = {
            f'H{i}.hpp': DSMMetrics(fan_in=25, fan_out=25, coupling=50, stability=0.5)
            for i in range(5)
        }
        all_headers = set(metrics.keys())
        stats = MatrixStatistics(
            total_headers=5,
            total_actual_deps=50,
            total_possible_deps=20,
            sparsity=30.0,
            avg_deps=10.0,
    ...
    def test_recommendations_clean_architecture(self, capsys: Any) -> None:
        ...
        metrics = {
            f'H{i}.hpp': DSMMetrics(fan_in=1, fan_out=1, coupling=2, stability=0.5)
            for i in range(5)
        }
        all_headers = set(metrics.keys())
        stats = MatrixStatistics(
...
class TestCompareDSMResults:
    ...
    def test_compare_cycle_changes(self) -> None:
        ...
        current = DSMAnalysisResults(
            ...
            sorted_headers=['A.hpp', 'B.hpp', 'C.hpp'],
            metrics={
                'A.hpp': DSMMetrics(2, 2, 4, 0.5),
                'B.hpp': DSMMetrics(2, 2, 4, 0.5),
                'C.hpp': DSMMetrics(1, 1, 2, 0.5),
            },
            cycles=[{'A.hpp', 'B.hpp'}],
            feedback_edges=[],
    ...
    def test_compare_coupling_changes(self) -> None:
        ...
        baseline = DSMAnalysisResults(
            ...
            metrics={
                'A.hpp': DSMMetrics(5, 5, 10, 0.5),
                'B.hpp': DSMMetrics(3, 3, 6, 0.5),
            },
            cycles=[],
            feedback_edges=[],
            layers=[],
        ...
        current = DSMAnalysisResults(
            ...
            sorted_headers=['A.hpp', 'B.hpp'],
            metrics={
                'A.hpp': DSMMetrics(8, 8, 16, 0.5),  # Coupling increased
                'B.hpp': DSMMetrics(2, 2, 4, 0.5),   # Coupling decreased
            },
            cycles=[],
            feedback_edges=[],
            layers=[],
...
class TestPrintDSMDelta:
    ...
    def test_print_delta_basic(self, capsys: Any, tmp_path: Path) -> None:
        ...
        current = DSMAnalysisResults(
            ...
            sorted_headers=['A.hpp', 'B.hpp'],
            metrics={
                'A.hpp': DSMMetrics(8, 7, 15, 0.467),
                'B.hpp': DSMMetrics(4, 5, 9, 0.556),
            },
            cycles=[{'A.hpp'}],
            feedback_edges=[],
            layers=[],
...
class TestDisplayDirectoryClusters:
    ...
    def test_display_clusters(self, capsys: Any, tmp_path: Path) -> None:
        ...
        all_headers = [
            str(module_a / 'file1.hpp'),
            str(module_a / 'file2.hpp'),
            str(module_b / 'file3.hpp'),
        ]
        
        header_to_headers = defaultdict(set, {
            all_headers[0]: {all_headers[1]},  # Intra-module
            all_headers[1]: {all_headers[2]},  # Inter-module
            all_headers[2]: set(),
        })
